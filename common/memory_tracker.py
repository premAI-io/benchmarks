...
class MemoryTracker:
    ...
    def _track_peak_memory(self, interval: float):
        ...
        mem_process: Process = PeakMemoryMeasureProcess(
            self.device_index, child_connection, interval
        )
        mem_process.start()
...
class PeakMemoryMeasureProcess(Process):
    def __init__(
        self, device_index: int, child_connection: Connection, interval: float
    ):
        super().__init__()
    ...
    def run(self):
        ...
        command = (
            f"nvidia-smi --query-gpu=memory.used --format=csv --id={self.device_index}"
        )
