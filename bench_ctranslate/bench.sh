...
setup() {
    ...
    if [[ "$MODEL_NAME" == "llama" ]]; then
        local model_pattern="$CURRENT_DIR/models/llama-2-7b-chat-ctranslate2-*"
    elif [[ "$MODEL_NAME" == "mistral" ]]; then
        local model_pattern="$CURRENT_DIR/models/mistral-7b-v0.1-instruct-ctranslate2-*"
    else
        echo "No such model is supported"
        exit 1
    fi

    matching_dirs=$(ls -d "$model_pattern" 2>/dev/null)

    if [ -n "$matching_dirs" ]; then
        echo "Already exists skipping setup"
    else
