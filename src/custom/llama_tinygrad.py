...
class Attention:
    ...
    def __call__(self, x: Tensor, cache_k: Optional[Tensor], cache_v: Optional[Tensor], start_pos: int, freqs_cis: Tensor, mask: Optional[Tensor], jit_ctx: Optional[Dict[Variable, int]] = None) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        xk = xk.reshape(xk.shape[0], xk.shape[1],
        ...
        if start_pos == 0:
            ...
            assert start_pos == (cache_k.shape[1].val if isinstance(cache_k.shape[1], Variable) else cache_k.shape[1]) == (cache_v.shape[1].val if isinstance(
                cache_v.shape[1], Variable) else cache_v.shape[1]), f"cache has wrong shape, {start_pos=}, {cache_k.shape[1]=}, {cache_v.shape[1]=}"
...
def concat_weights(models):
    def convert(name) -> Tensor:
        ...
        axis = 1 if name.startswith("tok_embeddings.") or name.endswith(
            ".attention.wo.weight") or name.endswith(".feed_forward.w2.weight") else 0
        lazy_tensors = [data.to(device=Device.DEFAULT)
                        for data in disk_tensors]
...
"""
    ...
    MODEL_PATH = args.model or Path(
        __file__).parents[1] / f"weights/LLaMA{LLAMA_SUFFIX}/{args.size}"
    ...
    for i in range(args.count):
        ...
        # with Timing("total ", enabled=True, on_exit=lambda x: f", {1e9/x:.2f} tok/sec"):
    ...
    toks = sum(times) / len(times) 
    print("\n\n============\navg toks/sec: " + str(toks))
