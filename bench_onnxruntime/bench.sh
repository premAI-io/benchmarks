...
########################################################################################################
# Script: bench.sh
# Description: This script runs benchmarks HF Optimum ONNX benchmark.
#
...
setup() {
    ...
    bash "$SCRIPT_DIR/setup.sh" "$MODEL_NAME" "$DEVICE"
}
...
docker run \
    ...
    -it anindyadeep/onnxruntime:latest \
    python3 -u "/mnt/benchmarks/bench_onnxruntime/bench.py" \
        --prompt "$PROMPT" \
